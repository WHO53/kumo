...
use glib::prelude::*;
use glib::Bytes;
use glutin::display::{AsRawDisplay, Display};
...
trait WebKitHandler {
    ...
    fn render_buffer(&mut self, engine_id: EngineId, buffer: Buffer);
...
impl WebKitHandler for State {
    fn render_buffer(&mut self, engine_id: EngineId, buffer: Buffer) {
        ...
        match buffer.downcast::<BufferDMABuf>() {
            Ok(dma_buffer) => webkit_engine.import_buffer(&self.protocol_states.dmabuf, dma_buffer),
            Err(buffer) => {
...
pub struct WebKitEngine {
    ...
    buffer: Option<(WlBuffer, BufferDMABuf)>,
    buffers_pending_release: [Option<(WlBuffer, BufferDMABuf)>; MAX_PENDING_BUFFERS],
...
impl WebKitEngine {
    pub fn new(
        ...
        // Create web view platform.
        let Display::Egl(egl_display) = display;
        let webkit_display =
            WebKitDisplay::new(queue.clone(), engine_id, egl_display.raw_display(), size, scale);
    ...
    fn import_buffer(&mut self, dmabuf_state: &DmabufState, buffer: BufferDMABuf) {
...
impl Engine for WebKitEngine {
    ...
    fn wl_buffer(&self) -> Option<&WlBuffer> {
        self.buffer.as_ref().map(|(wl_buffer, _)| wl_buffer)
    }
    ...
    fn buffer_released(&mut self, released_buffer: &WlBuffer) {
        ...
        for i in 0..self.buffers_pending_release.len() {
            let dmabuf = match &self.buffers_pending_release[i] {
                Some((wl_buffer, dmabuf)) if released_buffer == wl_buffer => dmabuf,
                Some(_) => continue,
                None => break,
            };
